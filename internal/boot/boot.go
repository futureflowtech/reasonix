...
func build(ctx context.Context, opts Options) (*BuildResult, error) {
	...
	// Model selection and reasoning preflight run on BOTH paths: a caller-owned
	// config still needs its model refs expanded and reasoning roles resolved,
	// and opts is rebound with the resolved selection. LoadForRoot previously
	// fed this; opts.Config now supplies the same shape directly.
